...
% R:result set(n*1)
% P:n*1
% r1,r2,m are parameters to add restrictions
...
function Rhat = LEAR(X,A,R,P,r1,r2,m,type)
...
for i = 1:l_a
    xx = find(A(:)==value_a(i));
	u(i,1) = sum(min(R(xx)./P(xx),m)) / n;   
    for j = 1:d
        u_x{i,j} = zeros(l_d(1,j),1);        
        for k = 1:l_d(j)
            l = find(X(:,j)==value_d{j}(k));
            ll = ismember(l,xx);
            u_x{i,j}(k) = sum(min(R(l(ll))./P(l(ll)),m));
        end
        u_x{i,j}(:,1) = u_x{i,j}(:,1) ./ n_d{j}(:);
...
for i = 1:l_a
    for j = 1:d
        ...
        for k = 1:l_d(j)
            l = find(A(:)==value_a(i));
            Vn{i,j}(k) = sum((min(R(l)./P(l),m)-u_x{i,j}(k)).^2);
        end
        Vn{i,j}(:) = Vn{i,j}(:) ./ (n-1);
